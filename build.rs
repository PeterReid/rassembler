...
const VEX_OP    : u32 = 0x0000_0001; // this instruction requires a VEX prefix to be encoded
const XOP_OP    : u32 = 0x0000_0002; // this instruction requires a XOP prefix to be encoded

// note: the first 4 in this block are mutually exclusive
const AUTO_SIZE : u32 = 0x0000_0004; // 16 bit -> OPSIZE , 32-bit -> None   , 64-bit -> REX.W/VEX.W/XOP.W
const AUTO_NO32 : u32 = 0x0000_0008; // 16 bit -> OPSIZE , 32-bit -> illegal, 64-bit -> None
const AUTO_REXW : u32 = 0x0000_0010; // 16 bit -> illegal, 32-bit -> None   , 64-bit -> REX.W/VEX.W/XOP.W
const AUTO_VEXL : u32 = 0x0000_0020; // 128bit -> None   , 256bit -> VEX.L
const WORD_SIZE : u32 = 0x0000_0040; // implies opsize prefix
const WITH_REXW : u32 = 0x0000_0080; // implies REX.W/VEX.W/XOP.W
const WITH_VEXL : u32 = 0x0000_0100; // implies VEX.L/XOP.L

const PREF_66   : u32 = WORD_SIZE; // mandatory prefix (same as WORD_SIZE)
const PREF_67   : u32 = 0x0000_0200; // mandatory prefix (same as SMALL_ADDRESS)
const PREF_F0   : u32 = 0x0000_0400; // mandatory prefix (same as LOCK)
const PREF_F2   : u32 = 0x0000_0800; // mandatory prefix (REPNE)
const PREF_F3   : u32 = 0x0000_1000; // mandatory prefix (REP)

const LOCK      : u32 = 0x0000_2000; // user lock prefix is valid with this instruction
const REP       : u32 = 0x0000_4000; // user rep prefix is valid with this instruction
const REPE      : u32 = 0x0000_8000;

const SHORT_ARG : u32 = 0x0001_0000; // a register argument is encoded in the last byte of the opcode
const ENC_MR    : u32 = 0x0002_0000; //  select alternate arg encoding
const ENC_VM    : u32 = 0x0004_0000; //  select alternate arg encoding
...
macro_rules! Ops {
    ( $bind:ident; $( $name:tt $(| $more:tt)* = [ $( $( $e:expr ),+ ; )+ ] )* ) => {
        fn $bind() -> Vec<(&'static str, &'static [Opdata])> {
            let mut map = Vec::new();
            const X: u8 = 0xFF;
            $({
                const DATA: &'static [Opdata] = &[$( OpInner!($( $e ),*) ,)+];
                map.push((stringify!($name), DATA));
                $(
                   map.push((stringify!($more), DATA));
                )*
            })+
            map
        }
...
pub fn hexify(xs: &[u8]) -> String {
    let mut result = String::new();
    for x in xs {
        result.push('\\');
        result.push('x');
        result.push(b"0123456789abcdef"[(*x as usize) >> 4] as char);
        result.push(b"0123456789abcdef"[(*x as usize) & 15] as char);
    }
    result
}
...
fn main() {
    ...
    /*lines.push("// This file has been generated by build.rs".to_string());
    lines.push("use assembler::{collect_args};".to_string());


    lines.push("use compiler::{Opdata, compile_op};".to_string());
    //lines.push("use x64::compile_op;".to_string());
    lines.push("use compiler::{Stmt, StmtBuffer};".to_string());
    lines.push("use parser::{Ident, Arg};".to_string());

    lines.push("".to_string());
    
    lines.push("pub struct FlaggedAssembler {".to_string());
    lines.push("    buffer: StmtBuffer,".to_string());
    lines.push("    prefixes: Vec<Ident>,".to_string());
    lines.push("}".to_string());
    
    lines.push("".to_string());
    
    lines.push("pub trait FlaggedAssemblerInternal {".to_string());
    lines.push("    fn new() -> Self;".to_string());
    lines.push("    fn get_stmts(&self) -> &[Stmt];".to_string());
    lines.push("    fn set_prefixes(&mut self, prefixes: Vec<Ident>);".to_string());
    lines.push("}".to_string());
    lines.push("impl FlaggedAssemblerInternal for FlaggedAssembler {".to_string());
    lines.push("    fn new() -> FlaggedAssembler { FlaggedAssembler { buffer: Vec::new(), prefixes: Vec::new() } }".to_string());
    lines.push("    fn get_stmts(&self) -> &[Stmt] { &self.buffer[..] }".to_string());
    lines.push("    fn set_prefixes(&mut self, prefixes: Vec<Ident>) { self.prefixes = prefixes; }".to_string());
    lines.push("}".to_string());
    
    lines.push("#[allow(non_snake_case)]".to_string());
    lines.push("impl FlaggedAssembler {".to_string());
    lines.push("    fn encode(&mut self, name: &str, data: &'static [Opdata], args: Vec<Arg>) {".to_string());
    lines.push("        compile_op(&mut self.buffer, name.to_string(), self.prefixes.clone(), args, data).expect(\"compile_op failed\")".to_string());
    lines.push("    }".to_string());
    */
    ...
    let ops = make_opmap();
    for &(name, opdatas) in &ops {
        assert!(opdatas.len()>0);
        
        let mut arg_counts = BTreeSet::new();
        for opdata in opdatas {
            arg_counts.insert(opdata.arg_count());
        }
        let max_arg_count = opdatas.iter().map(|x| x.arg_count()).max().expect("no opdatas!");
        
        let mut option_of_tuple = Vec::new();
        let mut arg_bindings = String::new();
        let mut arg_names = Vec::new();
        let mut pattern_so_far = String::new();
        let mut pattern_for_arg_count: HashMap<usize, String> = HashMap::new();
        
        let mut optional = false;
        for arg_idx in 0..max_arg_count {
            arg_bindings.push_str(", ");
            if arg_idx > 0 {
                pattern_so_far.push_str(", ");
            }
            if arg_counts.contains(&arg_idx) {
                pattern_for_arg_count.insert(arg_idx, close_parens(pattern_so_far.clone() + "None"));
                
                if !optional {
                    let mut remainder_name = String::new();
                    for rest_idx in arg_idx..max_arg_count {
                        remainder_name.push(chr(rest_idx));
                    }
                    arg_bindings.push_str(&remainder_name);
                    arg_bindings.push_str(": ");
                    
                    arg_names.push(remainder_name);
                }
                arg_bindings.push_str("Option<");
                pattern_so_far.push_str("Some(");
                
                let contains_tuple = arg_idx + 1 != max_arg_count;// !arg_counts.contains(&(arg_idx + 1));
                option_of_tuple.push(contains_tuple);
                if contains_tuple {
                    arg_bindings.push('(');
                    pattern_so_far.push('(');
                }
            ...
            if !optional {
                arg_names.push(format!("{}", chr(arg_idx)));
                arg_bindings.push_str(&format!("{}: ", chr(arg_idx)));
            }
        ...
        option_of_tuple.reverse();
        
        for x in option_of_tuple {
            if x {
                arg_bindings.push(')');
            }
            arg_bindings.push('>');
        ...
        for opdata in opdatas {
            lines.push(format!("            Opdata{{args: b{:?}, ops: b\"{}\", reg: {}, flags: {}}},", str::from_utf8(opdata.args).expect("bad args"), hexify(opdata.ops), opdata.reg, opdata.flags));
        }
        lines.push(format!("        ];"));
